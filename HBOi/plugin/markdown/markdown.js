...
(function (root, factory) {
  ...
  function convertSlides() {
    ...
    [].slice.call(sections).forEach(function (section) {
      ...
      const EMOJI_REGEXP = /:([a-z0-9_]+):(?:([2-5]x|lg):)?/g;
      const convertToEmoji = (_, name, size = '') => `<i class="twa twa-${size} twa-${name.replace(/_/g, '-')}"></i>`
      html = html.replace(EMOJI_REGEXP, convertToEmoji);
